...
const permissionForView = {
  ...
  reports: 'reports.view', users: 'users.manage', groups: 'groups.manage', audit: 'audit.view'
};
...
function showApp(user) {
  ...
  adminDivider.hidden = !['users.manage', 'groups.manage', 'audit.view'].some(has);
...
const views = {
  async dashboard() {
    ...
    content.innerHTML = `
      <div class="stat-grid">
        ${stat('Vendas de hoje', money(data.stats.salesToday), `${data.stats.salesCountToday} venda(s)`)}
        ${stat('Produtos ativos', number(data.stats.productCount), 'itens cadastrados')}
        ${stat('Custo do estoque', money(data.stats.inventoryCost), 'valor pelo custo atual')}
        ${stat('Estoque baixo', number(data.stats.lowStockCount), 'itens pedindo atenção', data.stats.lowStockCount ? 'warning' : '')}
      </div>
      <div class="panel-grid">
        <section class="panel"><div class="panel-header"><h2>Vendas nos últimos 7 dias</h2><span>VALOR TOTAL</span></div>
        ...
        <section class="panel"><div class="panel-header"><h2>Itens com estoque baixo</h2><span>${data.lowStock.length} EXIBIDOS</span></div>
      ...
      <section class="panel" style="margin-top:18px"><div class="panel-header"><h2>Vendas recentes</h2><span>ÚLTIMAS OPERAÇÕES</span></div>
        <div class="table-wrap">${salesTable(data.recentSales, false)}</div></section>`;
  },
  ...
  async stock() {
    ...
    content.innerHTML = `
      <div class="page-actions"><p>Acompanhe entradas, saídas e acertos de inventário.</p>${has('stock.manage') ? '<button class="button primary" id="new-movement">+ Nova movimentação</button>' : ''}</div>
  ...
  async users() {
    ...
    content.innerHTML = `<div class="page-actions"><p>Cadastre pessoas e vincule cada uma ao grupo adequado.</p><button class="button primary" id="new-user">+ Novo usuário</button></div>
      <section class="panel"><div class="table-wrap"><table class="data-table"><thead><tr><th>USUÁRIO</th><th>GRUPO</th><th>STATUS</th><th>CRIADO EM</th><th>AÇÕES</th></tr></thead><tbody>
      ${users.map(user => `<tr><td><div class="product-cell"><strong>${h(user.name)}</strong><span>${h(user.email)}</span></div></td><td>${h(user.group_name)}</td><td>${activeBadge(user.active)}</td><td>${dateTime(user.created_at)}</td><td><button class="button secondary small" data-edit-user="${user.id}">Editar</button></td></tr>`).join('')}</tbody></table></div></section>`;
  ...
  async groups() {
    ...
    content.innerHTML = `<div class="page-actions"><p>Defina exatamente quais áreas e ações cada perfil pode utilizar.</p><button class="button primary" id="new-group">+ Novo grupo</button></div>
      <section class="panel"><div class="table-wrap"><table class="data-table"><thead><tr><th>GRUPO</th><th>DESCRIÇÃO</th><th>PERMISSÕES</th><th>TIPO</th><th>AÇÕES</th></tr></thead><tbody>
      ${groups.map(group => `<tr><td><strong>${h(group.name)}</strong></td><td>${h(group.description || '—')}</td><td>${group.permissions.length} permissões</td><td>${group.is_system ? '<span class="badge">Padrão</span>' : '<span class="badge success">Personalizado</span>'}</td><td><button class="button secondary small" data-edit-group="${group.id}">Configurar</button></td></tr>`).join('')}</tbody></table></div></section>`;
  ...
  async audit() {
    ...
    content.innerHTML = `<div class="page-actions"><p>Registro das ações importantes realizadas no sistema.</p></div><section class="panel"><div class="table-wrap"><table class="data-table"><thead><tr><th>DATA</th><th>USUÁRIO</th><th>AÇÃO</th><th>ENTIDADE</th><th>IDENTIFICADOR</th><th>DETALHES</th></tr></thead><tbody>
      ${logs.map(log => `<tr><td>${dateTime(log.created_at)}</td><td>${h(log.user_name || 'Sistema')}</td><td><span class="badge">${h(log.action)}</span></td><td>${h(log.entity)}</td><td>${h(log.entity_id || '—')}</td><td title="${h(log.details)}">${h(log.details).slice(0, 75)}</td></tr>`).join('') || `<tr><td colspan="6">${empty('Sem registros', 'A trilha de auditoria aparecerá aqui.')}</td></tr>`}</tbody></table></div></section>`;
  }
};
...
function renderProducts(filtered = state.products) {
  content.innerHTML = `
    <div class="page-actions"><p>Catálogo, preços e níveis atuais de estoque.</p><div class="action-group">${has('products.manage') ? '<button class="button secondary" id="categories-button">Categorias</button><button class="button primary" id="new-product">+ Novo produto</button>' : ''}</div></div>
    ...
    <section class="panel"><div class="table-wrap"><table class="data-table"><thead><tr><th>PRODUTO</th><th>CATEGORIA</th><th>PREÇO</th><th>ESTOQUE</th><th>MÍNIMO</th><th>STATUS</th>${has('products.manage') ? '<th>AÇÕES</th>' : ''}</tr></thead><tbody id="products-body">${productRows(filtered)}</tbody></table></div></section>`;
  const apply = () => {
...
function productRows(products) {
  return products.map(item => `<tr><td><div class="product-cell"><strong>${h(item.name)}</strong><span>${h(item.sku)}${item.barcode ? ` · ${h(item.barcode)}` : ''}</span></div></td><td>${h(item.category_name || 'Sem categoria')}</td><td><strong>${money(item.price)}</strong><br><small>Custo ${money(item.cost)}</small></td><td><span class="badge ${item.stock <= item.min_stock ? 'warning' : 'success'}">${number(item.stock)}</span></td><td>${number(item.min_stock)}</td><td>${activeBadge(item.active)}</td>${has('products.manage') ? `<td><div class="table-actions"><button class="button secondary small" data-edit-product="${item.id}">Editar</button>${item.active ? `<button class="button danger small" data-delete-product="${item.id}">Desativar</button>` : ''}</div></td>` : ''}</tr>`).join('') || `<tr><td colspan="7">${empty('Nenhum produto encontrado', 'Ajuste os filtros ou cadastre um novo item.')}</td></tr>`;
}
...
function openProductModal(product = null) {
  ...
  openModal({ title: product ? 'Editar produto' : 'Novo produto', body: `<div class="form-grid">
    ...
    <label>Custo<input name="cost" type="number" min="0" step="0.01" value="${product?.cost ?? 0}" required></label><label>Preço de venda<input name="price" type="number" min="0" step="0.01" value="${product?.price ?? 0}" required></label>
    ${product ? '' : '<label>Estoque inicial<input name="stock" type="number" min="0" step="0.001" value="0" required></label>'}<label>Estoque mínimo<input name="minStock" type="number" min="0" step="0.001" value="${product?.min_stock ?? 0}" required></label>
    <label class="full">Descrição<textarea name="description">${h(product?.description || '')}</textarea></label>${product ? `<label class="check-row full"><input type="checkbox" name="active" ${product.active ? 'checked' : ''}> Produto ativo</label>` : ''}
...
function openMovementModal() {
  openModal({ title: 'Movimentar estoque', kicker: 'ENTRADA, SAÍDA OU AJUSTE', body: `<div class="form-grid">
    ...
    <label>Operação<select name="type" required><option value="ENTRY">Entrada</option><option value="EXIT">Saída</option><option value="ADJUSTMENT">Ajuste de inventário</option></select></label>
    <label>Quantidade<input name="quantity" type="number" min="0" step="0.001" required></label><label>Custo unitário<input name="unitCost" type="number" min="0" step="0.01" placeholder="Opcional"></label>
...
function renderCart() {
  ...
  $('#cart-summary').innerHTML = `<div class="cart-summary"><label>Desconto<input id="sale-discount" type="number" min="0" step="0.01" value="0"></label><label>Pagamento<select id="payment-method"><option>Dinheiro</option><option>PIX</option><option>Cartão de débito</option><option>Cartão de crédito</option></select></label><div class="summary-row"><span>Subtotal</span><span>${money(subtotal)}</span></div><div class="summary-row total"><span>Total</span><span id="sale-total">${money(subtotal)}</span></div><button class="button primary" id="finish-sale" ${items.length ? '' : 'disabled'}>Finalizar venda</button></div>`;
...
async function finishSale() {
  ...
  try {
    const sale = await api('/sales', { method: 'POST', body: JSON.stringify({ items, discount: $('#sale-discount').value, paymentMethod: $('#payment-method').value }) });
...
function salesTable(sales, actions) {
  return `<table class="data-table"><thead><tr><th>VENDA</th><th>DATA</th><th>OPERADOR</th><th>PAGAMENTO</th><th>ITENS</th><th>TOTAL</th><th>STATUS</th>${actions ? '<th>AÇÕES</th>' : ''}</tr></thead><tbody>${sales.map(sale => `<tr><td><strong>${h(sale.number)}</strong></td><td>${dateTime(sale.created_at)}</td><td>${h(sale.user_name)}</td><td>${h(sale.payment_method)}</td><td>${sale.item_count ?? '—'}</td><td><strong>${money(sale.total)}</strong></td><td><span class="badge ${sale.status === 'COMPLETED' ? 'success' : 'danger'}">${sale.status === 'COMPLETED' ? 'Concluída' : 'Cancelada'}</span></td>${actions ? `<td><div class="table-actions"><button class="button secondary small" data-view-sale="${sale.id}">Detalhes</button>${sale.status === 'COMPLETED' && has('sales.cancel') ? `<button class="button danger small" data-cancel-sale="${sale.id}">Cancelar</button>` : ''}</div></td>` : ''}</tr>`).join('') || `<tr><td colspan="8">${empty('Nenhuma venda', 'As vendas realizadas aparecerão aqui.')}</td></tr>`}</tbody></table>`;
}
...
async function renderReports(from = new Date(Date.now() - 30 * 86400000).toISOString().slice(0,10), to = new Date().toISOString().slice(0,10)) {
  ...
  content.innerHTML = `<div class="filters"><label>De<input id="report-from" type="date" value="${h(from)}"></label><label>Até<input id="report-to" type="date" value="${h(to)}"></label><button class="button primary" id="apply-report">Atualizar</button></div>
    <div class="stat-grid">${stat('Faturamento',money(report.summary.revenue),'vendas concluídas')}${stat('Vendas',number(report.summary.saleCount),'no período')}${stat('Ticket médio',money(report.summary.averageTicket),'por venda')}${stat('Estoque baixo',number(report.lowStock.length),'itens para repor',report.lowStock.length?'warning':'')}</div>
    <div class="panel-grid"><section class="panel"><div class="panel-header"><h2>Produtos mais vendidos</h2><span>POR QUANTIDADE</span></div><div class="table-wrap"><table class="data-table"><thead><tr><th>PRODUTO</th><th>QUANTIDADE</th><th>RECEITA</th></tr></thead><tbody>${report.topProducts.map(item=>`<tr><td><div class="product-cell"><strong>${h(item.name)}</strong><span>${h(item.sku)}</span></div></td><td>${number(item.quantity)}</td><td>${money(item.revenue)}</td></tr>`).join('')||`<tr><td colspan="3">${empty('Sem vendas no período','Altere as datas ou realize uma venda.')}</td></tr>`}</tbody></table></div></section>
    <section class="panel"><div class="panel-header"><h2>Reposição necessária</h2><span>ESTOQUE MÍNIMO</span></div><div class="panel-body"><div class="alert-list">${report.lowStock.map(item=>`<div class="alert-item"><div><strong>${h(item.name)}</strong><br><span>${h(item.sku)}</span></div><span class="badge warning">${number(item.stock)} / ${number(item.min_stock)}</span></div>`).join('')||empty('Tudo certo','Nenhum item abaixo do mínimo.')}</div></div></section></div>`;
...
function openGroupModal(group, permissions) {
  ...
  openModal({ title: group?'Configurar grupo':'Novo grupo', kicker:'PERMISSÕES DE ACESSO', body:`<div class="form-grid"><label>Nome<input name="name" value="${h(group?.name||'')}" required></label><label>Descrição<input name="description" value="${h(group?.description||'')}"></label></div>${modules.map(module=>`<h3 style="font-size:11px;margin:20px 0 8px">${h(module)}</h3><div class="permission-grid">${permissions.filter(item=>item.module===module).map(item=>`<label class="permission-item"><input type="checkbox" name="permissions" value="${h(item.code)}" ${group?.permissions.includes(item.code)?'checked':''}><span><strong>${h(item.label)}</strong><small>${h(item.code)}</small></span></label>`).join('')}</div>`).join('')}`, onSubmit:async form=>{
